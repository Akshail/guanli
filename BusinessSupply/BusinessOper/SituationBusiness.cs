...
{
    public class SituationBusiness
    {
        ...
        public static bool UpdateSituation(int situationId, DateTime time, string location, string situationName)
        {
            try
            {
                DbEntry.UsingTransaction(delegate
                {
                    Situation st = Situation.FindById(situationId);
                    st.Time = time;
                    st.Location = location;
                    st.SituationName = situationName;
                    st.Save();
        ...
        /// <returns></returns>
        public static IPagedSelector<Situation> GetSituations(int communicationId,int pagesize)
        {
            //查询
            try
            {
