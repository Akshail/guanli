...
{
    public partial class CheckAffairList_cztb_jtcy_Add : PageBase
    {
        private int peopleId//只有新增才传此值
        {
            get
            {
                try
                {
                    return Convert.ToInt32(Request.QueryString["peopleId"]);
                }
                catch
                {
                    return 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (familyId != 0)//修改
                {
                    ...
                    try
                    {
                        Family fam = Family.FindById(familyId);
